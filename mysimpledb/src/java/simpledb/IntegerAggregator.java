...
public class IntegerAggregator implements Aggregator {
	...
	public void mergeTupleIntoGroup(Tuple tup) {
		if (!grouping)
		{
			...
			{	
				...
				countsum.put(nogroupkey, new Countandsum());
			}
			Tuple update = groupbyagg.get(nogroupkey);
			update = aggregate(update, tup, countsum.get(nogroupkey), 0);
	...
	private Tuple createNoGroupTuple(Tuple tup)
	{
		...
		newtup.setField(0, new IntField(((IntField) tup.getField(aggfield)).getValue()));
		return newtup;
	}
