...
public class JoinOptimizer {
	...
	public static int estimateTableJoinCardinality(Predicate.Op joinOp,
		...
		int id1 = tableAliasToId.get(table1Alias);
		int id2 = tableAliasToId.get(table2Alias);
		HeapFile db1 = (HeapFile)Database.getCatalog().getDatabaseFile(id1);
		HeapFile db2 = (HeapFile)Database.getCatalog().getDatabaseFile(id2);

		TupleDesc td1 = db1.getTupleDesc();
		int index1 = td1.fieldNameToIndex(field1PureName);
		TupleDesc td2 = db2.getTupleDesc();
		int index2 = td2.fieldNameToIndex(field2PureName);

		int t1Distinct = stats.get(Database.getCatalog().getTableName(id1)).numDistinctValues(index1);
		int t2Distinct = stats.get(Database.getCatalog().getTableName(id2)).numDistinctValues(index2);
		int max = t2Distinct;
		if (t1Distinct > t2Distinct)
		{
			max = t1Distinct;
		}
		int returnval = (card1 * card2)/max; 
		if (!joinOp.equals(Predicate.Op.EQUALS))
		{
			returnval = (int) Math.ceil((card1 * card2)*0.7);
		}
		if(t1pkey)
		{
			if(t2pkey)
	...
	public Vector<LogicalJoinNode> orderJoins(HashMap<String, TableStats> stats,
		...
		{
			Set<Set<LogicalJoinNode>> joinsubsets = enumerateSubsets(joins, i);
			for (Set<LogicalJoinNode> subset: joinsubsets)
			{

				if(i == 1)
				{
					for(LogicalJoinNode ljNode: subset)
					{
						cc = computeCostAndCardOfSubplan(stats, filterSelectivities, ljNode, subset, Double.POSITIVE_INFINITY, pc);
				...
				else
				{
					...
					pc.addPlan(subset, bestCost, 0, v);
					for(LogicalJoinNode ljNode: subset)
					{
						cc = computeCostAndCardOfSubplan(stats, filterSelectivities, ljNode, subset, bestCost, pc);
						if(cc != null && pc.getCost(subset) > cc.cost)
						{
							bestCost = cc.cost;
							pc.addPlan(subset, bestCost, cc.card, cc.plan);
						}
					}
					if(i == numJoins)
					{
						joins = pc.getOrder(subset);
					}
				}
			}
		}
